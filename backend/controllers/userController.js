...
const {sendConfirmation,sendForgetPassword,resetPasswordEmail} = require('../middleware/nodemailer');
...
const count= await UserSchema.count() //estimatedDocumentCount() or countDocuments()
 ...
 if(users) return res.status(200).json({page:page,limit:limit,totalItems:count,users:users})
...
// }
 ...
 const disable =async(req, res)=>{
  try{
    await  UserSchema.findOne({ _id:req.params.id })
       .then(user => {
        if (user.isActive) {
          user.isActive = false;
          user.save();
